...
def run_single_experiment(config: Dict[str, Any], device: torch.device, batch_size: int, global_step_offset: int = 0) -> Dict[str, Any]:
    ...
    # Create per-tile model directory for this batch size
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    per_tile_model_dir = os.path.join(project_root, 'transformer', 'results', f'per_tile_bs_{batch_size}')
    ...
    for trimmed_idx, orig_grid_idx in zip(trimmed_tile_indices, orig_grid_indices):
        ...
        for epoch in range(config['n_epochs']):
            ...
            wandb.log({
                f"bs_{batch_size}/tile_{trimmed_idx}/train_loss": train_loss,
                f"bs_{batch_size}/tile_{trimmed_idx}/test_loss": test_loss,
                f"bs_{batch_size}/tile_{trimmed_idx}/train_rmse": train_rmse,
                f"bs_{batch_size}/tile_{trimmed_idx}/test_rmse": test_rmse,
                f"bs_{batch_size}/tile_{trimmed_idx}/train_mae": train_mae,
                f"bs_{batch_size}/tile_{trimmed_idx}/test_mae": test_mae,
                f"bs_{batch_size}/tile_{trimmed_idx}/train_emergence_rmse": train_emergence_rmse,
                f"bs_{batch_size}/tile_{trimmed_idx}/test_emergence_rmse": test_emergence_rmse,
                f"bs_{batch_size}/tile_{trimmed_idx}/train_emergence_mae": train_emergence_mae,
                f"bs_{batch_size}/tile_{trimmed_idx}/test_emergence_mae": test_emergence_mae,
                f"bs_{batch_size}/tile_{trimmed_idx}/learning_rate": current_lr,
                f"bs_{batch_size}/tile_{trimmed_idx}/global_step": global_step_offset
            })
    ...
    if valid_tiles:
        ...
        # Log aggregate metrics for this batch size under summary
        wandb.log({
            f"bs_{batch_size}/summary/avg_test_loss": avg_test_loss,
            f"bs_{batch_size}/summary/avg_test_rmse": avg_test_rmse,
            f"bs_{batch_size}/summary/avg_test_mae": avg_test_mae,
            f"bs_{batch_size}/summary/avg_test_emergence_rmse": avg_test_emergence_rmse,
            f"bs_{batch_size}/summary/avg_test_emergence_mae": avg_test_emergence_mae,
            f"bs_{batch_size}/summary/global_step": global_step_offset - 1
        })
    else:
...
def create_cross_trial_comparison_plots(all_results: Dict[float, Dict], config: Dict) -> None:
    """Create comprehensive comparison plots across all batch size trials with separate grids for each metric."""
    ...
    # Check if we have any batch sizes
    if not batch_sizes:
        print("No batch sizes found in results")
        return
    
    # Check what metrics are available in the results
    available_metrics = set()
    for bs in batch_sizes:
        if 'best_metrics' in all_results[bs]:
            for tile_metrics in all_results[bs]['best_metrics'].values():
                available_metrics.update(tile_metrics.keys())
    
    print(f"Available metrics: {available_metrics}")
    
    # Only plot metrics that exist in the results
    overall_metrics = []
    metric_labels = []
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']
    
    # Check which overall metrics are available
    if any('final_test_rmse' in metrics for bs, results in all_results.items() 
           for tile_idx, metrics in results.get('best_metrics', {}).items()):
        overall_metrics.append('final_test_rmse')
        metric_labels.append('RMSE')
    
    if any('final_test_mae' in metrics for bs, results in all_results.items() 
           for tile_idx, metrics in results.get('best_metrics', {}).items()):
        overall_metrics.append('final_test_mae')
        metric_labels.append('MAE')
    
    if any('best_test_loss' in metrics for bs, results in all_results.items() 
           for tile_idx, metrics in results.get('best_metrics', {}).items()):
        overall_metrics.append('best_test_loss')
        metric_labels.append('Loss')
    ...
    fig, axes = plt.subplots(1, len(overall_metrics), figsize=(5*len(overall_metrics), 8))
    if len(overall_metrics) == 1:
        axes = [axes]
    
    fig.suptitle('Performance Metrics Across Batch Sizes', fontsize=16, y=0.95)
    
    for i, (metric, label, color) in enumerate(zip(overall_metrics, metric_labels, colors)):
        ax = axes[i]
        
        # Calculate average metric across tiles for each batch size
        values = []
        for bs in batch_sizes:
            tile_metrics = all_results[bs].get('best_metrics', {})
            if tile_metrics:
                # Calculate average across all tiles
                metric_values = [tile_metrics[tile_idx].get(metric, float('nan')) 
                               for tile_idx in tile_metrics.keys()]
                # Filter out NaN values
                valid_values = [v for v in metric_values if not np.isnan(v)]
                if valid_values:
                    values.append(np.mean(valid_values))
                else:
        ...
        if any(not np.isnan(v) for v in values):
            bars = ax.bar(range(len(batch_sizes)), values, color=color, alpha=0.8)
            
            # Add value labels on bars
            for j, (bar, value) in enumerate(zip(bars, values)):
                if not np.isnan(value):
                    height = bar.get_height()
                    ax.text(j, height * 1.02, f'{value:.4f}', 
                           ha='center', va='bottom', fontsize=10, fontweight='bold')
            
            ax.set_xlabel('Batch Size', fontsize=12)
            ax.set_ylabel(f'{label} Value', fontsize=12)
            ax.set_title(f'Average {label}', fontsize=14)
            ...
            ax.text(0.5, 0.5, f'No valid {label} data', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=12)
            ax.set_title(f'{label} (No Data)', fontsize=14)
    
    plt.tight_layout(rect=[0, 0.05, 1, 0.93])
    wandb.log({"batch_size_comparison": wandb.Image(fig)})
    plt.close(fig)
    
    # Create summary table with available metrics
    summary_data = []
    for bs in batch_sizes:
        tile_metrics = all_results[bs].get('best_metrics', {})
        if tile_metrics:
            # Calculate averages across tiles
            row = {'batch_size': bs}
            for metric in overall_metrics:
                ...
                valid_values = [v for v in metric_values if not np.isnan(v)]
                if valid_values:
                    row[metric] = np.mean(valid_values)
                else:
    ...
    if summary_data:
        ...
        wandb.log({"batch_size_summary": wandb.Table(dataframe=summary_df)})
...
def create_epoch_mse_statistics_plot(all_results: Dict[float, Dict], config: Dict) -> None:
    ...
    for bs in batch_sizes:
        ...
        emergence_mse_values = []
        overall_mse_values = []
        
        # Collect MSE values from all tiles across epochs
        for tile_idx, tile_metrics in results.get('all_tile_metrics', {}).items():
            if 'test_emergence_rmses' in tile_metrics:
                emergence_mse_values.extend(tile_metrics['test_emergence_rmses'])
            if 'test_rmses' in tile_metrics:
                overall_mse_values.extend(tile_metrics['test_rmses'])
        
        # Calculate statistics
        if emergence_mse_values:
            emergence_stats.append({
                'mean': np.mean(emergence_mse_values),
                'std': np.std(emergence_mse_values),
                'min': np.min(emergence_mse_values),
                'max': np.max(emergence_mse_values),
                'count': len(emergence_mse_values)
            })
        else:
            emergence_stats.append({'mean': float('nan'), 'std': float('nan'), 
                                 'min': float('nan'), 'max': float('nan'), 'count': 0})
        
        if overall_mse_values:
            overall_stats.append({
                'mean': np.mean(overall_mse_values),
                'std': np.std(overall_mse_values),
                'min': np.min(overall_mse_values),
                'max': np.max(overall_mse_values),
                'count': len(overall_mse_values)
            })
        else:
            overall_stats.append({'mean': float('nan'), 'std': float('nan'), 
                               'min': float('nan'), 'max': float('nan'), 'count': 0})
    ...
    bars2 = ax2.bar(x_pos, overall_means, yerr=overall_stds, 
                    width=bar_width, alpha=0.8, color='#ff7f0e', capsize=5)
    
    # Add statistics text
    for i, (bs, stat) in enumerate(zip(batch_sizes, overall_stats)):
        if not np.isnan(stat['mean']):
            text = f"μ={stat['mean']:.4f}\nσ={stat['std']:.4f}\nmin={stat['min']:.4f}\nmax={stat['max']:.4f}\n(n={stat['count']})"
            ax2.text(i, stat['mean'] + stat['std'] + 0.001, text, 
                    ha='center', va='bottom', fontsize=8,
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
    ...
    for i, bs in enumerate(batch_sizes):
        stats_data.append({
            ...
            'emergence_mse_count': emergence_stats[i]['count'],
            ...
            'overall_mse_count': overall_stats[i]['count']
        })
...
def create_per_trial_mse_statistics(results: Dict, batch_size: int) -> None:
    ...
    if emergence_mse_values:
        ...
        emergence_stats = {'mean': float('nan'), 'std': float('nan'), 
                         'min': float('nan'), 'max': float('nan'), 'count': 0}
    ...
    if overall_mse_values:
        ...
        overall_stats = {'mean': float('nan'), 'std': float('nan'), 
                       'min': float('nan'), 'max': float('nan'), 'count': 0}
    
    valid_emergence = not np.isnan(emergence_stats['mean'])
    valid_overall = not np.isnan(overall_stats['mean'])
    
    if not valid_emergence and not valid_overall:
        print(f"No valid MSE data for BS {batch_size}")
        return
    ...
    if valid_emergence:
        ax1 = axes[0]
        emergence_epochs = list(range(1, len(emergence_mse_values) + 1))
        ax1.plot(emergence_epochs, emergence_mse_values, 'b-', alpha=0.7, linewidth=1)
        ax1.axhline(y=emergence_stats['mean'], color='red', linestyle='--', 
                   label=f'Mean: {emergence_stats["mean"]:.4f}')
        ax1.fill_between(emergence_epochs, 
                        emergence_stats['mean'] - emergence_stats['std'],
                        emergence_stats['mean'] + emergence_stats['std'],
                        alpha=0.3, color='red', label=f'±1σ: {emergence_stats["std"]:.4f}')
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Emergence MSE')
        ax1.set_title('Emergence MSE Across Epochs')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
    else:
        axes[0].text(0.5, 0.5, 'No Emergence MSE Data', 
                    ha='center', va='center', transform=axes[0].transAxes)
        axes[0].set_title('Emergence MSE (No Data)')
    
    # 2. Overall MSE trajectory over epochs
    if valid_overall:
        ax2 = axes[1]
        overall_epochs = list(range(1, len(overall_mse_values) + 1))
        ax2.plot(overall_epochs, overall_mse_values, 'g-', alpha=0.7, linewidth=1)
        ax2.axhline(y=overall_stats['mean'], color='red', linestyle='--', 
                   label=f'Mean: {overall_stats["mean"]:.4f}')
        ax2.fill_between(overall_epochs, 
                        overall_stats['mean'] - overall_stats['std'],
                        overall_stats['mean'] + overall_stats['std'],
                        alpha=0.3, color='red', label=f'±1σ: {overall_stats["std"]:.4f}')
        ax2.set_xlabel('Epoch')
        ax2.set_ylabel('Overall MSE')
        ax2.set_title('Overall MSE Across Epochs')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
    else:
        axes[1].text(0.5, 0.5, 'No Overall MSE Data', 
                    ha='center', va='center', transform=axes[1].transAxes)
        axes[1].set_title('Overall MSE (No Data)')
    
    # Add summary statistics text box
    stats_text = f"""Trial Statistics for BS {batch_size}:
...
Overall MSE:
  ...
  Count: {overall_stats['count']}"""
    ...
    # Log to wandb under the specific batch size tab
...
def main():
    print("Starting single batch size experiment...")
    ...
    # Best configuration based on emergence RMSE results
    config = {
        'num_pred': 12,
        'num_in': 110,
        'embed_dim': 64,
    ...
    search_type = "best_model"  # This is the best model configuration
    ...
    wandb.init(
        ...
        config=config,
        name="best_model",
        notes="Best model configuration: lr=0.001, bs=32, ff_dim=64, embed_dim=64, num_heads=8"
    )
    
    # Run experiment for single batch size
    ...
    print(f"\n{'='*50}")
    print(f"TRIAL 1/1: Testing batch size {batch_size}")
    print(f"{'='*50}")
    try:
        # Run experiment with global step offset
        results = run_single_experiment(config, device, batch_size, global_step_counter)
        all_results[batch_size] = results
        print(f"Creating MSE statistics for BS {batch_size}...")
        ...
        model_filename = f"transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{batch_size}_d{config['dropout']:.1f}.pth"
        model_path = os.path.join(models_dir, model_filename)
        torch.save(best_model_state, model_path)
        print(f"Best model saved to: {model_path}")
        print(f"Generating AR evaluation graphs for batch size {batch_size}...")
        ...
        for ar in [11698, 11726, 13165, 13179, 13183]:
            try:
                temp_output_dir = f"/tmp/ar_eval_bs{batch_size}"
                ...
                if plot_path and os.path.exists(plot_path):
                    wandb.log({f"bs_{batch_size}/ar_eval/AR_{ar}": wandb.Image(plot_path)}, step=None)
        ...
        model_artifact = wandb.Artifact(
            name=f"transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{batch_size}_d{config['dropout']:.1f}".replace('.', '_'),
            type="model",
        ...
        print(f"✓ Trial 1/1 completed successfully for batch size {batch_size}")
    except Exception as e:
        print(f"✗ ERROR in Trial 1/1 for batch size {batch_size}:")
    ...
    for ar in test_ars:
        try:
            ...
            if plot_path and os.path.exists(plot_path):
                ...
                wandb.log({f"bs_{batch_size}/ar_eval/AR_{ar}": wandb.Image(plot_path)})
            ...
            continue
    print(f"Completed AR evaluations for batch size {batch_size}: {len(successful_ars)} successful, {len(failed_ars)} failed")
    if successful_ars:
        eval_plots_artifact = wandb.Artifact(
            name=f"evaluation_plots_bs_{batch_size}".replace('.', '_'),
            type="evaluation_plots",
            description=f"AR comparison plots for batch size {batch_size}",
            metadata={
                "batch_size": batch_size,
    ...
    print(f"✓ Trial 1/1 fully completed including AR evaluations")
    ...
    for bs in sorted(all_results.keys()):
        ...
        trial_data = {
            'batch_size': bs,
            'train_loss_final': results.get('train_losses', [float('nan')])[-1] if results.get('train_losses') else float('nan'),
            'test_loss_final': results.get('test_losses', [float('nan')])[-1] if results.get('test_losses') else float('nan'),
        ...
        final_step = 1 * config['n_epochs'] - 1
        wandb.log({
            f'final_results/bs_{bs}/train_loss': trial_data['train_loss_final'],
            f'final_results/bs_{bs}/test_loss': trial_data['test_loss_final'],
            f'final_results/bs_{bs}/test_overall_rmse': trial_data['test_overall_rmse'],
            f'final_results/bs_{bs}/test_overall_r2': trial_data['test_overall_r2'],
            f'final_results/bs_{bs}/test_overall_mse': trial_data['test_overall_mse'],
            f'final_results/bs_{bs}/test_overall_mae': trial_data['test_overall_mae'],
            f'final_results/bs_{bs}/test_emergence_rmse': trial_data['test_emergence_rmse'],
            f'final_results/bs_{bs}/test_emergence_r2': trial_data['test_emergence_r2'],
            f'final_results/bs_{bs}/test_emergence_mse': trial_data['test_emergence_mse'],
            f'final_results/bs_{bs}/test_emergence_mae': trial_data['test_emergence_mae'],
            f'final_results/bs_{bs}/emergence_time_diff': trial_data['emergence_time_diff']
        }, step=final_step)
    ...
    all_models_artifact = wandb.Artifact(
        name="best_model",
        type="model",
        description=f"Best transformer model with optimal hyperparameters",
        metadata={
    ...
    for bs in [batch_size]:
        if bs in all_results:
            model_filename = f"transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{bs}_d{config['dropout']:.1f}.pth"
            model_path = os.path.join(models_dir, model_filename)
    ...
    summary_stats = {
        ...
        'best_overall_rmse_bs': batch_size,
        'best_emergence_rmse_bs': batch_size,
        'best_timing_accuracy_bs': batch_size,
        'all_trials_rmse_range': 0.0,
        'avg_emergence_rmse': all_results[batch_size].get('emergence_rmse', 0.0) if batch_size in all_results else 0.0,
        'avg_overall_rmse': all_results[batch_size].get('overall_rmse', 0.0) if batch_size in all_results else 0.0
    }
    wandb.log(summary_stats)
    best_overall = batch_size
    best_emergence = batch_size
    best_time_error = batch_size
    wandb.log({
        'best_overall_bs': best_overall,
        'best_emergence_bs': best_emergence,
        'best_timing_bs': best_time_error,
        'best_overall_rmse': all_results[batch_size].get('overall_rmse', 0.0) if batch_size in all_results else 0.0,
        'best_emergence_rmse': all_results[batch_size].get('emergence_rmse', 0.0) if batch_size in all_results else 0.0,
        'best_time_error': all_results[batch_size].get('emergence_time_diff', 0.0) if batch_size in all_results else 0.0
    })
    summary_text = f"""
    BEST CONFIGURATIONS:
    - Overall Performance: BS={best_overall} (RMSE: {all_results[batch_size].get('overall_rmse', 0.0):.4f})
    - Emergence Prediction: BS={best_emergence} (RMSE: {all_results[batch_size].get('emergence_rmse', 0.0):.4f})  
    - Timing Accuracy: BS={best_time_error} (Error: {all_results[batch_size].get('emergence_time_diff', 0.0):.2f} hrs)
    """
    wandb.log({"best_configurations_summary": summary_text})
    print(f"\n{'='*50}")
    print("EXPERIMENT COMPLETE!")
    print(f"Best overall performance: BS={best_overall} (RMSE: {all_results[batch_size].get('overall_rmse', 0.0):.4f})")
    print(f"Best emergence prediction: BS={best_emergence} (RMSE: {all_results[batch_size].get('emergence_rmse', 0.0):.4f})")
    print(f"Best timing accuracy: BS={best_time_error} (Error: {all_results[batch_size].get('emergence_time_diff', 0.0):.2f} hrs)")
    print(f"{'='*50}")
    print(f"\n{'='*50}")
    ...
    model_filename = f"transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{batch_size}_d{config['dropout']:.1f}.pth"
    model_path = os.path.join(models_dir, model_filename)
    artifact_name = f"transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{batch_size}_d{config['dropout']:.1f}".replace('.', '_')
    if os.path.exists(model_path):
    ...
    print(f"\nWandb artifact collection: bs_search_all_models")
    print(f"  Contains: {len([bs for bs in [batch_size] if bs in all_results])} models")
    print(f"\nTo download models from wandb:")
    print(f"  # Download individual model")
    print(f"  artifact = wandb.use_artifact('your-entity/sar-emergence/transformer_t12_r4_i110_n3_h64_e300_b8_d0_3:latest')")
    print(f"  artifact_dir = artifact.download()")
    print(f"  # Download all models")
    print(f"  artifact = wandb.use_artifact('your-entity/sar-emergence/bs_search_all_models:latest')")
    print(f"  artifact_dir = artifact.download()")
    print(f"\nTo load a model:")
    print(f"  model_state = torch.load('{models_dir}/transformer_t{config['time_window']}_r{config['rid_of_top']}_i{config['num_in']}_n{config['num_layers']}_h{config['embed_dim']}_e{config['n_epochs']}_b{batch_size}_d{config['dropout']:.1f}.pth')")
    print(f"{'='*50}")
    wandb.finish()
