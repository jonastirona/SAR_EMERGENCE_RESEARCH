...
def run_single_experiment(config: Dict[str, Any], device: torch.device, learning_rate: float, weight_decay: float, global_step_offset: int = 0, batch_size: int = 128) -> Dict[str, float]:
    ...
    for epoch in range(config['n_epochs']):
        ...
        with torch.no_grad():
            ...
            # Log per-epoch metrics with weight decay prefix and unique global step
            wd_str = f"{weight_decay:.0e}"
            metrics_to_log = {
                f'wd_{wd_str}/train_loss': epoch_train_loss,
                f'wd_{wd_str}/test_loss': epoch_test_loss,
                f'wd_{wd_str}/test_emergence_rmse': test_emergence_metrics['emergence_rmse'],
                f'wd_{wd_str}/test_emergence_mse': test_emergence_metrics.get('emergence_mse', float('nan')),
                f'wd_{wd_str}/test_emergence_mae': test_emergence_metrics.get('emergence_mae', float('nan')),
                f'wd_{wd_str}/test_emergence_r2': test_emergence_metrics.get('emergence_r2', float('nan')),
                f'wd_{wd_str}/test_overall_rmse': test_emergence_metrics.get('overall_rmse', float('nan')),
                f'wd_{wd_str}/test_overall_mse': test_emergence_metrics.get('overall_mse', float('nan')),
                f'wd_{wd_str}/test_overall_mae': test_emergence_metrics.get('overall_mae', float('nan')),
                f'wd_{wd_str}/test_overall_r2': test_emergence_metrics.get('overall_r2', float('nan')),
                f'wd_{wd_str}/learning_rate': optimizer.param_groups[0]['lr'],
                f'wd_{wd_str}/epoch': epoch,
                ...
                'epoch': epoch,
                'current_weight_decay': weight_decay,  # Add current wd for filtering
                'trial_id': f'wd_{wd_str}'  # Add trial identifier
            }
...
def main():
    ...
    config = {
        ...
        'weight_decays': [1e-3, 1e-4, 1e-5]  # Weight decay values to search
    }
    ...
    wandb.init(
        ...
        entity=wandb_entity,
        config=config,
        name="weight_decay_search_all",
        notes="Searching for optimal weight decay (1e-3, 1e-4, 1e-5) with fixed batch size 128, constant learning rate (0.001), dropout (0.3), fixed attention heads (4), and 3 layers"
    )
