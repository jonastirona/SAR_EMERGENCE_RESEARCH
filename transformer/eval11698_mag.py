...
from functions_no_pytorch import min_max_scaling, find_closest_fits_frame_to_NOAA_record, lstm_ready, recalibrate, smooth_with_numpy, emergence_indication, calculate_metrics, add_grid_lines, highlight_tile
...
pth_file = glob.glob('./ApJ_paper_lstm/best/*.pth') # Assuming there's only one .pth file and its naming follows the specific pattern
filename = pth_file[0]
...
rid_of_top = 1; num_in = 96 # REDEFINED BE CAREFUL / # Now you can use these variables in your script
...
test_AR = 11698 # and the secondary will be 13165 and if I fix it, third: 13183
...
NOAA_first = datetime(2013, 3, 15, 0, 0, 0); NOAA_first_record = mdates.date2num(NOAA_first) # Convert to matplotlib date format
NOAA_second = datetime(2013, 3, 17, 0, 0, 0); NOAA_second_record = mdates.date2num(NOAA_second) # Convert to matplotlib date format
...
starting_tile = 46-rid_of_top*9 # Change this CHANGEE
future = num_pred - 1
all_metrics = []
...
for i in range(7):
    ...
    pred = true.copy() #THIS IS NEW CAUSE NO PYTORCH
    ...
    ax0.plot(time_cut_mpl, mag_flux[starting_tile+i, last_known_idx-before_plot+1:], color='gray', linestyle='--', alpha=0.7)
    ...
    ax0.axvline(x=NOAA_second_record, color='grey', linestyle='--')
    #if i == 5: ax0.legend(['Predicted', 'Observed', 'Observed (Smooth)', 'First Prediction', r'NOAA $1^{st}$ Record', 'After Emergence'], fontsize=7, loc='upper left', bbox_to_anchor=(0.0, -1))
  # Legend for each plot (optional)
    ...
    first = True
    for k in range(len(d_pred) - 1): # Now, plot using time_cut_mpl as x-values
...
save_path = './ApJ_paper_lstm/best/mag_AR{}_{}.pdf'.format(test_AR,os.path.splitext(os.path.basename(pth_file[0]))[0])
