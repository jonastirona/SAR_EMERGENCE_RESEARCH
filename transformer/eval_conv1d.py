...
def evaluate_models_for_ar(test_AR, lstm_path, transformer_path, transformer_params, output_dir):
    ...
    try:
        ...
        # ?? UPDATED: Initialize transformer model with temporal conv parameter
        trfm = SpatioTemporalTransformer(
            ...
            use_pre_mlp_norm=True,     # Enable for saved model
            use_temporal_conv=use_temporal_conv  # ?? NEW: Pass temporal conv parameter
        ).to(device)
        
        print(f"  Debug: Loading model state dict with temporal_conv={use_temporal_conv}")
        ...
        for i in range(7):
            ...
            ax4.plot(tnum[before_plot:before_plot+len(true)], transformer_errors, 'r-', label=f'Transformer{"(+TC)" if use_temporal_conv else ""}')
