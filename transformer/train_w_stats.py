...
if model_name == 'lstm':
    ...
    # Use exact parameters inferred from original model analysis
    model = SpatioTemporalTransformer(
        input_dim=5,           # 4 power maps + 1 magnetic flux
        seq_len=110,           # num_in parameter
        embed_dim=64,          # hidden_size parameter
        num_heads=1,           # Inferred from original model (unusual but correct)
        ff_dim=128,            # Inferred from original model
        num_layers=2,          # Actual layers (not 3 as in filename)
        output_dim=12,         # num_pred parameter
        dropout=0.1,           # Inferred from original model
        use_pre_mlp_norm=True  # Original model has pre_mlp_norm layer
    ).to(device)
...
with open(result_file_path, "w") as file:
    ...
    for AR_ in range(len(ARs)):
        ...
        for tile in range(tiles):
            ...
            print("X_test_final shape:", X_test_final.shape, "y_test shape:", y_test.shape)
        # Move data to GPU
            X_train_final = X_train_final.to(device)
        y_train = y_train.to(device)
            X_test_final = X_test_final.to(device)
            y_test = y_test.to(device)
            results = training_loop_w_stats(n_epochs=n_epochs,lstm=model,optimiser=optimiser,loss_fn=loss_fn,
