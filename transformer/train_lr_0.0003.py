...
def run_single_experiment(config: Dict[str, Any], device: torch.device, learning_rate: float, global_step_offset: int = 0) -> Dict[str, float]:
    ...
    batch_size = 128  # Fixed batch size as per experiment requirements
    ...
    scheduler = CosineWarmupScheduler(optimizer, warmup_epochs=30, max_epochs=config['n_epochs'])
    loss_fn = nn.MSELoss()
...
def main():
    ...
    # Only run a single trial with lr=0.0003
    lr = 0.0003
    print(f"\n{'='*50}")
    print(f"SINGLE TRIAL: Testing learning rate {lr}")
    print(f"{'='*50}")
    try:
        # Run experiment
        results = run_single_experiment(config, device, lr, 0)
        # Store results for subsequent evaluation summaries
        ...
        print(f"✓ Single trial completed successfully for learning rate {lr}")
    except Exception as e:
        print(f"✗ ERROR in single trial for learning rate {lr}:")
