...
test_AR = 11726 # and the secondary will be 13165 and if I fix it, third: 13183


# Argument parser
parser = argparse.ArgumentParser(description="Evaluate model results for Active Region (AR).")
parser.add_argument('--pth_file', type=str, required=True, help="Path to the .pth file containing the model weights.")
parser.add_argument('--save_path', type=str, required=True, help="Path to save the output plot.")
parser.add_argument('--model_name', type=str, choices=['lstm', 'st_transformer'], default='lstm', help="Model name: 'lstm' or 'st_transformer'.")
...
if model_name == 'lstm':
    ...
    raise ValueError("Invalid model name. Choose either 'lstm' or 'st_transformer'.")
...
model.load_state_dict(new_state_dict) # Load the corrected state_dict into your model
...
for i in range(7):
    ...
    ### Validation
    ### torch.Size([150, 72, 5]) torch.Size([150, 12]) tile,size,power_maps,intensities,num_in,num_pred
    
    # print(starting_tile+i,size,inputs.shape,intensities.shape,num_in,num_pred)
...
plt.suptitle('model Results for AR{} (TW = {}h, RoT = {}, In = {}h)'.format(test_AR,num_pred,rid_of_top,num_in), y=0.99)
