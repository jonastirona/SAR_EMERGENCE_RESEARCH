...
BASE_PATH = "../"
DATA_PATH = os.path.join(BASE_PATH, "SAR_EMERGENCE_RESEARCH/data")
RESULTS_PATH = os.path.join(BASE_PATH, "SAR_EMERGENCE_RESEARCH/lstm/results")
...
def load_ar_data(ar_num, size, rid_of_top):
    ...
    try:
        ...
        trim_slice = slice(rid_of_top * size, -rid_of_top * size if rid_of_top > 0 else None)
...
def main(config):
    ...
    x_train, y_train, input_size = prepare_dataset(train_ars, config['size'], config['rid_of_top'], config['num_in'], config['num_pred'])
    
    print("Loading and preparing test data...")
    test_ars = [11462, 11521, 11907, 12219, 12271, 12275, 12567]
    x_test, y_test, _ = prepare_dataset(test_ars, config['size'], config['rid_of_top'], config['num_in'], config['num_pred'])
    ...
    # --- Model & Optimizer ---
    model = LSTM(input_size, config['hidden_size'], config['num_layers'], config['num_pred'], dropout=config['dropout']).to(device)
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=config['learning_rate'])
    scheduler = StepLR(optimizer, step_size=config['n_epochs'] // 10, gamma=0.9)
...
def parse_args():
    ...
    if len(sys.argv) != 9:
        print("Usage: python train_one_epoch.py <num_pred> <rid_of_top> <num_in> <num_layers> <hidden_size> <n_epochs> <learning_rate> <dropout>")
        sys.exit(1)
