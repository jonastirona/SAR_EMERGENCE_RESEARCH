...
def eval_AR_emergence_with_plots(
    ...
    for test_AR in [11698, 11726, 13165, 13179, 13183]:
        ...
        for i in range(7):
            ...
            # bring back the original values of flux
            predicted = median_scalers["flux"].inverse_transform(
                np.concatenate((nan_array, pred)).reshape(-1, 1)
            )
            observed = median_scalers["flux"].inverse_transform(
                np.concatenate((mag_before_pred, true)).reshape(-1, 1)
            )
            ...
            ax0.set_ylim(
                [
                    min(np.nanmin(predicted), np.nanmin(observed)),
                    max(np.nanmax(predicted), np.nanmax(observed)),
                ]
            )
            ax0.grid(
            ...
            for j in range(len(d_true) - 1):  # Now, plot using time_cut_mpl as x-values
                ...
                if current_color == "r" and first == True:
                    ...
                    true_emergence = readable_time[1]
                ax1.plot(
            ...
            for k in range(len(d_pred) - 1):  # Now, plot using time_cut_mpl as x-values
                ...
                if current_color == "r" and first == True:
                    ...
                    pred_emergence = readable_time[1]
