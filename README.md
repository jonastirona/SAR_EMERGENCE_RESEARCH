...
│ ├── individual_train.py # Single model training
│ ├── functions.py # Helper functions, models, loss functions
│ ├── test_models_denormalized.py # Model evaluation with plots
│ ├── test_emergence_loss.py # Unit tests for emergence-aware loss
│ └── results/ # Training outputs
└── README.md

### Loss Functions

The project supports three loss function types:

- **`value`**: Standard MSE loss on predicted values
- **`hybrid`**: Weighted combination of value MSE + derivative MSE (controlled by `alpha`)
- **`emergence`**: Emergence-aware loss combining value MSE + derivative MSE + differentiable emergence timing loss (controlled by `alpha` and `gamma`). Uses curriculum annealing for sigmoid steepness `k` (10 → 100 over epochs) and rising-edge detection for onset isolation.
