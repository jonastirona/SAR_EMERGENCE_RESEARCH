...
def get_piece_means(input_cube, size): 
    """Calculate mean of each tile for each time step"""
    parts = size**2
...
def split_image(image, size): 
    """Split a given image into equal-size square pieces"""
...
def load_and_preprocess_ar_eval_template(test_AR, data_path, raw_data_path, rid_of_top, size):
    """Load and preprocess AR data with per-AR normalization and geometric offset calculation"""
    base = f'{data_path}/AR{test_AR}'
    raw_base = f'{raw_data_path}/AR{test_AR}'
    
    # Load flattened data (for model training)
    ...
    # Load raw data (for geometric offset calculation)
    cont_raw = np.load(os.path.join(raw_base, f'intensities{test_AR}.npz'), allow_pickle=True)

    pm23, pm34, pm45, pm56, time_arr = (
        power['arr_0'], power['arr_1'], power['arr_2'], power['arr_3'], power['arr_4']
    )
    mf = mag['arr_0']; ii = cont['arr_0']
    ii_raw_3d = cont_raw['arr_0']  # This is 3D (time, height, width)

    # Convert raw 3D data to 2D tile format using same preprocessing
    ii_raw = get_piece_means(ii_raw_3d, size)  # Now 2D (tiles, time)

    # Store raw data BEFORE trimming for proper denormalization
    ii_raw_full = ii_raw.copy()  # Use the raw data, not flattened
    
    sl = slice(rid_of_top*size, -rid_of_top*size)
    pm23, pm34, pm45, pm56 = pm23[sl,:], pm34[sl,:], pm45[sl,:], pm56[sl,:]
    mf = mf[sl,:]; ii = ii[sl,:]
    ii_raw = ii_raw[sl,:]  # Apply same slice to raw data
    mf[np.isnan(mf)] = 0; ii[np.isnan(ii)] = 0
    ii_raw[np.isnan(ii_raw)] = 0

    # Calculate geometric offset: offset = raw - flat
    offset = ii_raw - ii
    ...
    # Return both normalized and raw data for denormalization, plus geometric offset
    return inputs, ii, time_arr, (mp, Mp, mm, Mm, mi, Mi), ii_raw_full, offset
...
def evaluate_single_ar_with_denorm(test_AR, 
    ...
    try:
        ...
        # Load data with normalization stats for denormalization and geometric offset
        inputs, ii, time_arr, norm_stats, ii_raw_full, offset = load_and_preprocess_ar_eval_template(test_AR, data_path, raw_data_path, rid_of_top, size)
        ...
        for i in range(7):
            ...
            # Add geometric offset back to predictions and ground truth
            offset_slice = offset[tile_idx, -true.shape[0]:]
            p_l_raw = p_l_raw + offset_slice
            p_t_raw = p_t_raw + offset_slice
            true_raw = true_raw + offset_slice
            ...
            # Use raw data for proper range calculation
        ...
        # Save as PDF
        plot_dir = Path(output_dir) / 'raw_ar_evaluations'
...
def main():
    parser = argparse.ArgumentParser(description='Redo Experiment B AR Evaluation with Raw Data Denormalization')
    parser.add_argument('--data_path', type=str, default='/mmfs1/project/mx6/jst26/final/data', help='Data directory')
    parser.add_argument('--raw_data_path', type=str, default='/project/mx6/jst26/final/raw_data', help='Raw data directory')
    ...
    print("="*100)
    print("REDO EXPERIMENT B AR EVALUATION WITH RAW DATA DENORMALIZATION")
    print("="*100)
    ...
    print(f"PDF outputs saved to: {output_dir}/raw_ar_evaluations/")
