...
def train_transformer_with_full_attention(
    ...
    nhead=8,
    dropout=0.1,
    use_temporal_conv=True  # NEW: Conv1D control parameter
):
    """
    Train transformer using full attention capabilities with Conv1D support
    """
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f'Training on: {device}')
    print(f"Parameters: pred={num_pred}, layers={num_layers}, hidden={hidden_size}")
    print(f"Conv1D Temporal Layers: {'✅ ENABLED' if use_temporal_conv else '❌ DISABLED'}")
    ...
    # Load data
    print('Loading data for enhanced transformer with Conv1D...')
    ...
    # Initialize enhanced transformer model with Conv1D
    model = SARTransformerLocalTile(
        ...
        use_temporal_conv=use_temporal_conv  # NEW: Conv1D control
    ).to(device)
    ...
    if use_temporal_conv:
        ...
        print(f"Conv1D Temporal Parameters: {conv_params:,}")
    ...
    conv_suffix = "_conv1d" if use_temporal_conv else "_no_conv1d"
    results_file = os.path.join(output_dir, f"enhanced_transformer{conv_suffix}_training_results.txt")
    
    print(f"\n=== TRAINING ENHANCED TRANSFORMER WITH Conv1D {conv_suffix.upper()} ===")
    
    with open(results_file, "w") as file:
        file.write(f"Enhanced Transformer Training Results (Conv1D {conv_suffix})\n")
        file.write(f"Model: d_model={hidden_size}, layers={num_layers}, heads={nhead}\n")
        file.write(f"Conv1D: {'ENABLED' if use_temporal_conv else 'DISABLED'}\n")
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % 50 == 0:
                conv_status = "✅ ENABLED" if use_temporal_conv else "❌ DISABLED"
                print(f"Epoch {epoch}: loss={avg_loss:.6f}, lr={scheduler.get_last_lr()[0]:.6f}, Conv1D={conv_status}")
                
                # NEW: Log temporal scale weights
                if hasattr(model.transformer, 'scale_weights'):
                    scale_weights = torch.softmax(model.transformer.scale_weights, dim=0)
                    print(f'Temporal Scale Weights: {scale_weights.detach().cpu().numpy()}')
            ...
            if avg_loss < best_loss:
                best_loss = avg_loss
                best_model_path = os.path.join(output_dir, f"best_enhanced_transformer{conv_suffix}.pth")
                torch.save(model.state_dict(), best_model_path)
    
    # Save final model
    model_path = os.path.join(output_dir, f"enhanced_transformer{conv_suffix}_t{num_pred}_r{rid_of_top}_i{num_in}_n{num_layers}_h{hidden_size}_e{n_epochs}_l{learning_rate}.pth")
...
def main():
    parser = argparse.ArgumentParser(description='Train Enhanced Attention Transformer with Conv1D Support')
    parser.add_argument('--data_path', type=str, default='/project/mx6/jst26/SAR_EMERGENCE_RESEARCH/data', help='Data directory path')
    parser.add_argument('--output_dir', type=str, default='/project/mx6/jst26/SAR_EMERGENCE_RESEARCH/sarang_code/enhanced_transformer_results', help='Output directory')
    ...
    parser.add_argument('--use_temporal_conv', action='store_true', default=True, help='Enable Conv1D temporal layers')
    parser.add_argument('--no_temporal_conv', action='store_true', help='Disable Conv1D temporal layers')
    
    args = parser.parse_args()
    
    # Handle Conv1D flag logic
    if args.no_temporal_conv:
    ...
    print("="*70)
    print("ENHANCED TRANSFORMER TRAINING WITH Conv1D SUPPORT")
    print("="*70)
    print("Key Features:")
    print("- Multi-head attention with relative positional encoding")
    print("- Emergence-aware loss function")
    print("- Multi-scale Conv1D temporal pattern extraction")
